...
export class SmartInstall {
  ...
  async run(options: SmartInstallOptions = {}): Promise<SmartInstallOutput> {
    ...
    // as a shared singleton at startup (see createOptimizerRuntime()), so
    // without this, a caller's projectRoot argument was silently ignored and
    // install ran against the server process's own launch directory instead
    // (matches the fix already applied to smart_build/lint/typecheck/test).
