...
export class SmartCacheTool extends EventEmitter {
  ...
  private scheduleWriteBack(): void {
    ...
    // Same real fix as cache-replication.ts's/cache-invalidation.ts's timers
    // (see their own comments): only reachable when a caller explicitly
    // requests `writeMode: 'write-back'` (the class's own default is
    // `'write-through'`, which never schedules this at all), but unref'd
    // anyway so an in-flight write-back timer never delays process exit.
