...
export class CacheInvalidationTool extends EventEmitter {
  ...
  private startScheduler(): void {
    ...
    // REAL BUG FIX (not silently ported): this timer starts unconditionally
    // from the constructor -- vendor's own code never called `.unref()`,
    // meaning merely CONSTRUCTING this tool (which
    // `createOptimizerRuntime()` always does, whether or not
    // `cache_invalidation` is ever actually called) kept the Node process
    // alive forever on a 10-second interval. `.unref()` preserves the real
    // scheduled-invalidation behavior when the process is alive for other
    // reasons, while letting the process exit naturally otherwise --
    // exactly the same class of fix as this merge's `defaultProjectRoot`
    // fixes elsewhere, a real defect fixed rather than replicated.
  ...
  private scheduleLazyProcessing(): void {
    ...
    // Same real fix as startScheduler() above -- a self-clearing timer, so
    // less severe, but still unref'd so an in-flight lazy-invalidation timer
