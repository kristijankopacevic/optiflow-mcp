...
export class CacheReplicationTool extends EventEmitter {
  ...
  private restartSyncTimer(): void {
    ...
    // REAL BUG FIX (not silently ported): all 4 of this class's background
    // timers start unconditionally from `startBackgroundTasks()`, called
    // unconditionally from the constructor -- vendor's own code never
    // called `.unref()` on any of them, meaning merely CONSTRUCTING this
    // tool (which `createOptimizerRuntime()` always does, whether or not
    // `cache_replication` is ever actually called) kept the Node process
    // alive forever on a 1s/5s/10s/5min interval cadence. `.unref()`
    // preserves the real sync/heartbeat/health-check/snapshot behavior when
    // the process is alive for other reasons (e.g. the real stdio MCP
    // server, which stays alive on its own transport regardless), while
    // letting a short-lived process (tests, the smoke-test script) exit
    // naturally -- the same class of fix as this merge's `defaultProjectRoot`
    // fixes elsewhere.
  ...
  private restartHeartbeatTimer(): void {
    ...
    // Same real fix as restartSyncTimer() above.
