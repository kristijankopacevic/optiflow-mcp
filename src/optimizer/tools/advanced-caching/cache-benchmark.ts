...
// PATH RECONCILIATION (see src/optimizer/paths.ts's header): vendor's
// default output directory here was `join(homedir(), '.hypercontext',
// 'reports')` -- replaced with optiflow's own `~/.optiflow/optimizer/reports`
// convention. Also fixes a real pre-existing vendor gap: the default path
// was never `mkdirSync`-created before `writeFileSync` used it below (would
// throw ENOENT on a clean machine with no prior `.hypercontext` directory).
import { getOptimizerReportsDir } from '../../paths.js';
...
export class CacheBenchmark {
  ...
  private async generateReport(
    ...
    // Real fix for a real pre-existing vendor gap (see the import comment
    // above): create the directory before writing, since neither the old
    // default nor the new one is guaranteed to already exist.
