...
// Factory function, added for consistency with every other ported tool's
// get*Tool(cache, tokenCounter, metrics) singleton-factory convention. This
// tool had no factory upstream (only `new AnomalyExplainer(...)` inline
// inside runAnomalyExplainer above); vendor's own src/tools/intelligence/index.ts
// marks AnomalyExplainer "Implementation pending" / never dispatches it.
let anomalyExplainerInstance: AnomalyExplainer | null = null;

export function getAnomalyExplainerTool(
  cache: CacheEngine,
  tokenCounter: TokenCounter,
  metricsCollector: MetricsCollector
): AnomalyExplainer {
  if (!anomalyExplainerInstance) {
    anomalyExplainerInstance = new AnomalyExplainer(cache, tokenCounter, metricsCollector);
  }
  return anomalyExplainerInstance;
}
