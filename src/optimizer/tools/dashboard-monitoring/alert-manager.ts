...
export class AlertManager {
  ...
  private async silenceAlerts(
    ...
    // Auto-cleanup expired silences. Not constructor-started (this only
    // fires on an explicit `silence` call, so it doesn't share the
    // higher-severity "always hangs the process" defect checkpoint 7 found
    // in cache-replication/cache-invalidation's constructors), but unref'd
    // anyway for the same hygiene reason checkpoint 7 unref'd
    // cache-invalidation's lazyProcessTimer / smart-cache's writeBackTimer:
    // a short-lived process (tests, the smoke-test script) should still be
    // able to exit before `options.silenceDuration` elapses.
    setTimeout(
