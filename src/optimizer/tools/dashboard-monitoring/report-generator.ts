 ...
 *
 * DEFERRED (see dashboard-monitoring/index.ts's header for the full
 * category-level finding): unlike this category's other 9 files, this one
 * exports ONLY the `ReportGenerator` class -- no `export function
 * create*`/`get*` factory and no `export const *_TOOL_DEFINITION`/`*Tool`
 * object anywhere in the file (verified directly: `grep -n "^export "`
 * shows nothing below the class body except more `export interface`
 * types). There is no `name`/`inputSchema` to build a real MCP `Tool`
 * definition from without inventing one vendor itself never wrote, and
 * vendor's own `src/server/index.ts` never dispatches a `report_generator`/
 * `report-generator` case either (grepped directly, confirmed absent) --
 * genuinely incomplete/unfinished vendor code, not an oversight in this
 * merge. Left excluded from tsconfig.json's compile graph and NOT exported
 * from this category's index.ts, matching the spirit of the
 * `smart_typescript` precedent (the underlying reason differs -- a missing
 * schema here, vs. a missing runtime dependency there -- but the treatment
 * is the same: copy the real, compiling class in for future completion,
 * don't fabricate a dispatch surface for it today).
 */
