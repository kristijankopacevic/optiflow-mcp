...
class HealthCheckExecutor {
  ...
  private async executeHttpCheck(
    ...
    // On-demand, self-clearing (both the success and catch paths below call
    // clearTimeout) -- not the constructor-started, always-running class of
    // timer checkpoint 7 found in cache-replication/cache-invalidation. Still
    // unref'd for the same hygiene reason checkpoint 7 unref'd those tools'
    // on-demand timers too: a short-lived process shouldn't have to wait out
    // this timeout if `fetch` itself hangs without honoring the abort signal.
