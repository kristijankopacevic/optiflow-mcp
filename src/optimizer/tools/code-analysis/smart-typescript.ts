...
export class SmartTypeScript {
  ...
  async run(
    ...
    // as a singleton, so without this re-read the schema's advertised
    // `projectRoot` option was silently ignored (this class's `run()` never
    // read `options.projectRoot` at all -- only the standalone CLI runner
    // passed it into the constructor). Same fix as smart_install/smart_docker/
    // smart_logs (build-systems) and smart_build/smart_lint/smart_typecheck.
