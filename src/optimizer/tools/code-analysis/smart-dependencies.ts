...
// DEPENDENCY RECONCILIATION: vendor parses .ts/.tsx via
// `@typescript-eslint/typescript-estree` and everything else via
// `@babel/parser`. typescript-estree's own peerDependencies pin
// `typescript` to `>=4.8.4 <6.1.0`; optiflow already depends on
// `typescript@^7.0.2` (see package.json) for the rest of this merge, so
// adding typescript-estree here would be a real ERESOLVE peer-dependency
// conflict against a version range this repo can't satisfy. Both
// `extractImports`/`extractExports` below only touch plain ESTree-shaped
// node fields (`ImportDeclaration.source.value`, `.specifiers[].local/
// imported.name`, `ExportNamedDeclaration.declaration`, `ExportDefault
// Declaration`, `ExportAllDeclaration.source`, `node.loc.start.line`) --
// fields Babel's own TS-aware parse produces identically to
// typescript-estree's ESTree output for these specific node types. So
// every extension (including .ts/.tsx) now routes through `@babel/parser`
// with the `typescript` plugin enabled, dropping the
// `@typescript-eslint/typescript-estree` dependency entirely rather than
// installing a version-incompatible peer.
...
export class SmartDependenciesTool {
  ...
  private analyzeFile(filePath: string, cwd: string): DependencyNode | null {
    try {
      ...
      // Parse via Babel for every supported extension -- see this file's
      // top-of-file DEPENDENCY RECONCILIATION comment for why
      // typescript-estree isn't used here despite vendor's original having
      // a separate .ts/.tsx branch for it. `ext` stays computed above for
      // the extension-resolution logic elsewhere in this file.
      let ast: any;
      try {
