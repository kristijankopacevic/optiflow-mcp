// Ported verbatim from vendor/token-optimizer-mcp/src/core/compression-engine.ts
// (MIT-licensed -- see THIRD_PARTY_LICENSES.md). No hardcoded paths here (pure
// brotli wrapper over Node's builtin `zlib`), so nothing to reconcile against
// src/optimizer/paths.ts. Ported as a new sibling in src/optimizer/core/
// because analytics/optimization-storage.ts (this checkpoint's Part 1) is the
// first ported module to depend on it.
