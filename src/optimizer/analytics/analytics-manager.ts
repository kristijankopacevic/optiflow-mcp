// Ported verbatim from vendor/token-optimizer-mcp/src/analytics/analytics-manager.ts
// (MIT-licensed -- see THIRD_PARTY_LICENSES.md). No hardcoded paths of its
// own (delegates all storage I/O to the injected `AnalyticsStorage`, whose
// real implementation -- `SqliteAnalyticsStorage` -- is the piece with the
// path to reconcile; see analytics-storage.ts's own header).
