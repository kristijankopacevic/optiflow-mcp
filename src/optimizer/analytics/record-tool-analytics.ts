 ...
 *
 * Ported from vendor/token-optimizer-mcp/src/analytics/record-tool-analytics.ts
 ...
 *
 * WIRING DECISION (src/optimizer/server.ts's `createOptimizerRuntime()`):
 * vendor's real `src/server/index.ts` calls this on the result of EVERY tool
 * dispatch -- "THE ONE PLACE EVERY TOOL RESULT PASSES THROUGH" per that
 * file's own comment -- passing a `baselineResult` (the undisclosed,
 * pre-`discloseResult()` payload) so `savingsMeasured` can be computed from
 * two real materialized payloads. optiflow has no `discloseResult()` /
 * `mcpEvidence` subsystem (that whole undisclosed-content pipeline is out of
 * this checkpoint's scope), so this merge wires the SAME "every tool call"
 * breadth (every entry in `createOptimizerRuntime()`'s registry is wrapped,
 * not just the 5 analytics tools) but with NO `baselineResult` and empty
 * `attribution` -- an honest degraded mode, not a fabricated one:
 * `savingsMeasured` stays `false` for every recorded entry (no claim of a
 * verified transport reduction is ever made), so `verifiedTransportDelta()`
 * contributes 0 for all of them and `get_optimization_report`'s "Verified
 * saved" / "Direct API price" fields will legitimately read 0 / "not priced"
 * until a real disclosure/baseline pipeline is ported. What IS real:
 * `totalOperations` and `totalOptimizedTokens` (the actual returned-payload
 * token count, per `savings-classification.ts`'s "observed-return-only"
 * classification) are genuine per-call measurements from day one, giving the
 * 5 analytics tools real usage data instead of an always-empty store.
 */
