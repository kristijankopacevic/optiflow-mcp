// Ported from vendor/token-optimizer-mcp/src/analytics/native-provider-usage.ts
// (MIT-licensed -- see THIRD_PARTY_LICENSES.md).
//
// NOT YET WIRED (like optimization-storage.ts, see that file's header): the
// only real consumer of `readNativeProviderUsage` is
// `vendor/token-optimizer-mcp/src/server/dashboard-analytics.ts`, part of
// the still-deferred dashboard-monitoring category (see server.ts's own
// header) -- copied in per the merge plan's Part 1 file list, not
// constructed or exposed anywhere in `createOptimizerRuntime()` yet.
//
// PATH RECONCILIATION: `~/.codex/sessions`, `~/.claude/projects`, and
// `~/.gemini/tmp` (scanned further down in this file) are OTHER TOOLS' own
// well-known transcript directories -- reading them is the same kind of
// arbitrary-external-path access `smart_read` already does elsewhere in this
// server, not one of optiflow's own `~/.token-optimizer*` paths, so those
// three are deliberately left untouched. `hookEvidenceFiles()`'s
// `~/.token-optimizer/{wiki,unrooted}` and `~/.token-optimizer/projects.jsonl`
// ARE genuinely our own paths, though (the same wiki dir/projects registry
// `src/optimizer/paths.ts already reconciled for `tools/intelligence/wiki-*`)
// -- reconciled below via `getOptimizerWikiDir()`/`getOptimizerUnrootedDir()`/
// `getOptimizerProjectsJsonlPath()` for the real default-home case. The
// `homeDirectory` override this file accepts is a test-fixture seam distinct
// from `OPTIFLOW_HOME` (it lets a caller point the whole multi-client scan --
// codex/claude/gemini transcripts included -- at one fake directory tree for
// a hermetic test), so that override path still resolves relative to the
// literal passed-in `home` rather than through the OPTIFLOW_HOME-aware
// helpers.
import fs from 'fs';
import os from 'os';
...
function hookEvidenceFiles(
  home: string,
  sinceMs: number,
  useReconciledDefaults: boolean
): string[] {
  // See this file's header: the real OPTIFLOW_HOME-aware paths for the
  // no-override case, or the literal `home`-relative join for the
  // `homeDirectory` test-fixture override.
  const roots = useReconciledDefaults
    ? [getOptimizerWikiDir(), getOptimizerUnrootedDir()]
    : [
        path.join(home, '.optiflow', 'optimizer', 'wiki'),
        path.join(home, '.optiflow', 'optimizer', 'unrooted'),
      ];
  const registry = useReconciledDefaults
    ? getOptimizerProjectsJsonlPath()
    : path.join(home, '.optiflow', 'optimizer', 'projects.jsonl');
...
export async function readNativeProviderUsage(
  ...
  const hookFiles = hookEvidenceFiles(home, sinceMs, !options.homeDirectory);
  const records = (
    await Promise.all([
