...
// PATH RECONCILIATION (see src/optimizer/paths.ts's header): the original
// default root here was `path.join(homedir(), '.token-optimizer', 'backups')`,
// replaced by optiflow's own `~/.optiflow/optimizer/backups` convention.
import { getOptimizerBackupsDir } from '../paths.js';
 ...
 *
 * `OPTIFLOW_HOME` (honored by `getOptimizerBackupsDir()` via
 * `getOptiflowHome()`) already redirects this for test isolation; this
 * env var is a finer-grained override for backups specifically.
 */
const BACKUP_DIR_ENV = 'OPTIFLOW_OPTIMIZER_BACKUP_DIR';

/** Default root: outside every working tree, so a backup can never be committed. */
export const BACKUP_ROOT = getOptimizerBackupsDir();
