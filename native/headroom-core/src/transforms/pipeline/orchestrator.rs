...
impl CompressionPipeline {
    ...
    pub fn run(
        ...
        // which way it falls; correctness is the same. When the
        // `parallel` feature is off (e.g. wasm32-unknown-unknown, which
        // has no OS thread pool for rayon to spin up), both phases run
        // serially on the calling thread — same result, no parallelism.
        #[cfg(feature = "parallel")]
    ...
    /// Run every offload's bloat estimator in parallel (serially when the
    /// `parallel` feature is off). Returns scores in the same order as the
    /// input slice.
    #[cfg(feature = "parallel")]
