...
impl HfTokenizer {
    ...
    ///
    /// Gated behind the `remote-tokenizer` feature (pulls in `hf-hub`,
    /// which does blocking network I/O and filesystem-cache access — both
    /// meaningless on `wasm32-unknown-unknown`). `from_bytes`/`from_file`
    /// remain available unconditionally for local/bundled tokenizers.
    #[cfg(feature = "remote-tokenizer")]
